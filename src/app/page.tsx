...
export default function Home() {
  ...
  const openButtonSelection = (e: Event) => {
  ...
  const openMenu = (e: Event, type = "") => {
    e.stopPropagation()
    setSelectedButton(type)
  }
  return (
    <main className="h-100 w-100">
      <div className='h-100 w-100 px-0 quicks-dark'>
        <div className="input-group">
          ...
          <div id='mainMenuButtonContainer'>
            <button className={selectedButton == "" ? "btn rounded-circle" : selectedButton == "inbox" ? "btn btn-inbox rounded-circle" : "btn btn-task rounded-circle" } id="selectedButton" onClick={openButtonSelection}>
              {
                selectedButton == "" ? 
            ...
            <div className="d-flex button-selection" id="buttonSelection">
              <div className={buttonStatus == 0 ? "init" : buttonStatus == 1 && selectedButton == "" ? "show" : selectedButton != "task" ? "hide" : "hide-no-anim"}>
                <h6 className='text-white text-center'>Task</h6>
                <button className='btn rounded-circle' id="btnTask" onClick={(e) => {openMenu(e, "task")}}>
                  <svg width="26" height="20" viewBox="0 0 26 20" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <div className={buttonStatus == 0 ? "init" : buttonStatus == 1 && selectedButton == "" ? "show" : selectedButton != "inbox" ? "hide" : "hide-no-anim"}>
                <h6 className='text-white text-center'>Inbox</h6>
                <button className='btn rounded-circle' id="btnInbox" onClick={(e) => {openMenu(e, "inbox")}}>
